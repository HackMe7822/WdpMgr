...
function Scan-AndClear([bool]$listOnly) {
    $script:scanned = 0; $script:found = 0; $script:cleared = 0; $script:failed = 0
    ...
    $callback = {
        ...
        $procId = 0
        [void][WinAffinity]::GetWindowThreadProcessId($hWnd, [ref]$procId)
        $procName = "unknown"
        ...
        Write-Host ("[FOUND] PID {0} ({1}) - `"{2}`" - affinity={3}" -f $procId, $procName, $title, (AffinityName $affinity))
    ...
    Write-Host ""
    if ($listOnly) {
        Write-Host "Scanned $($script:scanned) windows. Found $($script:found) with capture-exclusion set."
    } else {
        Write-Host "Scanned $($script:scanned) windows. Found $($script:found) with capture-exclusion set. Cleared $($script:cleared), failed $($script:failed)."
    }
}
